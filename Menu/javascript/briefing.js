...
function setupEventListeners() {
  ...
  if (statusFilter) statusFilter.addEventListener('change', renderBriefingList);
  if (searchBriefing) searchBriefing.addEventListener('input', renderBriefingList);

  const closeBriefingDetailBtn = document.getElementById('closeBriefingDetailBtn');
  const detailEditBtn = document.getElementById('detailEditBtn');
  const detailDeleteBtn = document.getElementById('detailDeleteBtn');

  if (closeBriefingDetailBtn) closeBriefingDetailBtn.addEventListener('click', closeDetailModal);
  ...
  if (confirmDeleteBtn) confirmDeleteBtn.addEventListener('click', deleteBriefing);
    ...
    <div class="briefing-item" onclick="openBriefingDetail(${briefing.id})">
      ...
      <div class="briefing-item-meta">
        ...
        <span class="briefing-channels">
          ${briefing.channels.map(channel => `<span class="channel-tag">${getChannelName(channel)}</span>`).join('')}
        </span>
      </div>
    </div>
  `).join('');
    ...
    <div class="detail-content">
      <div class="detail-section">
        ...
        <div class="detail-grid">
          <div class="detail-item"><label>Descrição</label><p>${briefing.description}</p></div>
          <div class="detail-item"><label>Objetivos e Prioridades</label><p>${briefing.objectives}</p></div>
          <div class="detail-item"><label>Call to Action</label><p class="cta-highlight">${briefing.cta}</p></div>
        </div>
      </div>
      ...
      <div class="detail-section">
        ...
        <div class="detail-grid">
          <div class="detail-item"><label>Público-Alvo</label><p>${briefing.target}</p></div>
          <div class="detail-item"><label>Análise da Concorrência</label><p>${briefing.competition || 'Não informado'}</p></div>
          <div class="detail-item"><label>Análise de Mercado</label><p>${briefing.market || 'Não informado'}</p></div>
        </div>
      </div>
      ...
      <div class="detail-section">
        ...
        <div class="detail-grid">
          <div class="detail-item"><label>Principais Mensagens</label><p>${briefing.messages}</p></div>
          <div class="detail-item"><label>Tom de Voz</label><p>${getToneText(briefing.tone)}</p></div>
          <div class="detail-item">
            <label>Canais de Distribuição</label>
            <div class="channels-list">
              ${briefing.channels.map(channel => `<span class="channel-badge">${getChannelName(channel)}</span>`).join('')}
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="detail-section">
        <h3>Execução</h3>
        <div class="detail-grid">
          <div class="detail-item"><label>Orçamento</label><p>${briefing.budget ? `R$ ${briefing.budget.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}` : 'Não definido'}</p></div>
          <div class="detail-item"><label>Cronograma</label><p>${briefing.timeline ? briefing.timeline.replace(/\n/g, '<br>') : 'Não definido'}</p></div>
          <div class="detail-item"><label>Período</label><p>${briefing.startDate ? `${formatDate(briefing.startDate)} à ${formatDate(briefing.endDate)}` : 'Não definido'}</p></div>
        </div>
      </div>
    </div>
  `;
...
function closeDetailModal() {
  ...
  setTimeout(() => { briefingDetailModal.style.display = 'none'; }, 300);
}
...
function openNewBriefingModal() {
  ...
  // defaults pros selects do modal
  const tone = document.getElementById('briefingTone');
  const status = document.getElementById('briefingStatus');
  if (tone) { tone.value = ""; tone.dispatchEvent(new Event('change', { bubbles: true })); }
...
function editBriefing(id) {
  ...
  // sync UI dos selects custom
  document.getElementById('briefingTone').dispatchEvent(new Event('change', { bubbles: true }));
  document.getElementById('briefingStatus').dispatchEvent(new Event('change', { bubbles: true }));

  document.querySelectorAll('input[name="channels"]').forEach(cb => {
    cb.checked = briefing.channels.includes(cb.value);
  });
...
function closeModal() {
  ...
  setTimeout(() => { briefingModal.style.display = 'none'; }, 300);
}
...
/* -------------------------
   CUSTOM SELECT (PADRÃO PLANEJAMENTO)
   - abre com .open (bate com seu CSS)
-------------------------- */
function initializeCustomSelects() {
  ...
  customSelects.forEach(cs => {
    ...
    const hiddenSelect = cs.querySelector('select');
    if (!selected || !items || !hiddenSelect) return;

    if (cs.dataset.ready === "1") return;
    cs.dataset.ready = "1";
    ...
    function syncFromSelect() {
      ...
      if (opt) selected.querySelector('span').textContent = opt.textContent;

      options.forEach(o => o.classList.remove('selected'));
      const active = [...options].find(o => (o.dataset.value ?? "") === (hiddenSelect.value ?? ""));
    ...
    selected.addEventListener('click', (e) => {
      ...
      document.querySelectorAll('.custom-select.open').forEach(openCs => {
        if (openCs !== cs) openCs.classList.remove('open');
      });
...
function getStatusText(status) {
  const statusMap = { 'draft': 'Rascunho', 'in-progress': 'Em Andamento', 'completed': 'Concluído' };
...
function getChannelName(channel) {
  const channelMap = { 'instagram': 'Instagram', 'facebook': 'Facebook', 'twitter': 'Twitter', 'linkedin': 'LinkedIn' };
